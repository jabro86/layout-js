...
class DragDrop {
  ...
  addGlass(fCancel: (() => void) | undefined) {
    if (!this._glassShowing) {
      const width = document.documentElement !== null ? document.documentElement.clientWidth : 0;
      const height = document.documentElement !== null ? document.documentElement.clientHeight : 0;
      const glassRect = new Rect(0, 0, width, height);
